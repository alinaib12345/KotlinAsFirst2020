...
fun center(list: MutableList<Double>): MutableList<Double> {
    val meanNumber = mean(list)
    for (i in 0 until list.size) {
        val element = list[i]
        list[i] = element - meanNumber
    }
    return list
}
...
fun roman(n: Int): String {
    ...
    for (j in lenght downTo 0) {
        ...
        when (x) {
            ...
            3 -> romanNumber.append(alphabet[i - 1], alphabet[i - 1], alphabet[i - 1])
            4 -> romanNumber.append(alphabet[i - 1], alphabet[i])
            5 -> romanNumber.append(alphabet[i])
            6 -> romanNumber.append(alphabet[i], alphabet[i - 1])
...
fun russian(n: Int): String {
    val finalPhrase = mutableListOf<String>()
    ...
    val teens = listOf(
        "одиннадцать", "одиннадцать", "двенадцать", "двенадцать", "тринадцать", "четырнадцать", "пятнадцать",
        "шестнадцать", "семнадцать", "восемнадцать", "девятнадцать"
    )
    val wrongTeens = listOf(
        "десять один", "десять одна", "десять два", "десять две", "десять три", "десять четыре", "десять пять",
        "десять шесть", "десять семь", "десять восемь", "десять девять"
    )
    for (i in wrongTeens.indices) if (wrongTeens[i] in final)
        final = final.replace(wrongTeens[i], teens[i])
    val wrong = listOf(
        "одиннадцать тысячи", "одиннадцать тысяча", "двенадцать тысячи", "двенадцать тысяча", "тринадцать тысячи",
        "четырнадцать тысячи"
    )
    val right = listOf(
        "одиннадцать тысяч", "одиннадцать тысяч", "двенадцать тысяч", "двенадцать тысяч", "тринадцать тысяч",
        "четырнадцать тысяч"
    )
    for (k in wrong.indices) if (wrong[k] in final)
        final = final.replace(wrong[k], right[k])
    return final
}
